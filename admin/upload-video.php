...
if (isset($_POST["submit"])) {
    ...
    // Corrected the path by removing the accidental space before the "/"
    $target_folder = "../assets/video"; // Adjust the path to your existing 'video' folder

    // Use the $target_folder variable to construct the file path
    $run = move_uploaded_file($video_tmp, $target_folder . "/" . $video_name);

    if ($run) {
        echo "<div class='alert alert-success'>Video uploaded successfully!</div>";
    } else {
        // More descriptive error message
        echo "<div class='alert alert-danger'>Error uploading video. Please check permissions and file path.</div>";
    }
}
?>
